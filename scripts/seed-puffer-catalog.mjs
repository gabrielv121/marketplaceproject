#!/usr/bin/env node
/**
 * Upsert puffer/down outerwear from data/puffer-outerwear-catalog.json.
 * Run: node scripts/seed-puffer-catalog.mjs
 */
...
const raw = await readFile(path.join(root, "data", "puffer-outerwear-catalog.json"), "utf8");
...
console.log(`Upserting ${rows.length} puffer/down item(s):`);
...
const { count: pufferTagged } = await sb
  .from("catalog_products")
  .select("id", { count: "exact", head: true })
  .eq("published", true)
  .contains("tags", ["puffer"]);
console.log(`Published rows tagged puffer: ${pufferTagged ?? 0}`);
