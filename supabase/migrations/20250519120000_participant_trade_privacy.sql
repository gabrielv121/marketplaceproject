-- Buyers and sellers only interact with EXCH.; counterparty PII must not be readable via client APIs.

drop policy if exists "trades_select_parties" on public.p2p_trades;
...
begin
  ...
  -- Counterparty user ids are not exposed to participants.
...
revoke all on function public.get_trade_for_participant(uuid) from public;
grant execute on function public.get_trade_for_participant(uuid) to authenticated;

revoke all on function public.list_my_trades() from public;
grant execute on function public.list_my_trades() to authenticated;

comment on function public.get_trade_for_participant is
  'Trade detail for buyer or seller with counterparty PII redacted. Direct p2p_trades SELECT is not allowed for participants.';

comment on function public.list_my_trades is
  'Account trade list for the signed-in user; omits counterparty ids and role-specific secrets.';
